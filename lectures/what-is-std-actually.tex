...
$\mathbf{std}(\MVec{x})$ with average deviation. It is more
accurate to describe it in geometric terms.
...
\vspace*{1em}
\noindent If we simply average the absolute
deviations, we get Mean Absolute Deviation (MAD):
\[
    \mathbf{MAD} = \dfrac{0.46+0.51+0.29+0.18+1.22+0.79+1.27+1.92}{8} = 0.83,
\]
not $1$. The gap exists because one entry, $1.92$,
contributes $1.92^2 \approx 3.696$ to the sum of squares ---
almost half the total, while the other seven values together
contribute the rest. Squaring inflates the influence of large
deviations, so $\mathbf{std}$ ends up pulled above the
``typical'' value one would gather from the data.
...
to $r$ has the same magnitude as $\overline{\MVec{x}}$.
It does \textbf{not} claim that individual entries
resemble $r$ --- only that the total squared deviation is conserved.

\subsection*{Finance analogy}
A \textbf{fixed deposit} scheme has $\mathbf{std} = 0$
(no fluctuation, ever). A \textbf{real stock} fluctuates
unevenly --- big swings some periods, small ones others.
The idea of $\mathbf{std}$ answers the question:

\begin{quote}
\emph{If the same total risk (or the magnitude)
were spread perfectly evenly instead of lumped unevenly,
what size should each period's swing have to be?}
\end{quote}
...
regular seesaw is the flat instrument (fixed deposit scheme)
carrying identical total risk.

\section*{Conclusion}
It is important to understand that $\mathbf{std}$ is not
``how much things usually differ'' --- it is ``the flat,
constant-swing version of data that has the same total squared
spread.''

When deviations are roughly equal in size (like the seesaw),
the flat version nicely matches our intuition. When one or
two entries dominate (like $1.92$ in the example above),
$\mathbf{std}$ still measures the real total spread correctly,
however, it no longer looks like an ``average-sized'' deviation.
Actually, it isn't one.
...
\begin{itemize}
    ...
    \item It means that if all fluctuations were smoothed out
          into regular and equal-sized deviations of
          magnitude $\pm r$, the total combined impact
          (variance) would remain identical.
\end{itemize}
...
\begin{quote}
    ...
    uniform magnitude that preserves the total magnitude
    of the vector, that uniform magnitude would be $1.0$.
\end{quote}

\section*{The Mathematical Reasoning}
...
\[
    \|v\|^2 = \sum_i v_i^2 = \sum_i r^2 = n r^2
\]
and therefore,
...
\noindent Setting $\|v\| = \|\overline{\MVec{x}}\|$,
\[
    r\sqrt{n} = \|\overline{\MVec{x}}\|
        ...
        = \mathbf{std}(\MVec{x})
\]
\noindent $\mathbf{std}(\MVec{x})$ is the unique uniform magnitude
